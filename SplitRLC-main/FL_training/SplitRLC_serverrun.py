...
server = Sever(0, config.SERVER_ADDR, config.SERVER_PORT, 'LeNet')
...
if offload:
	...
	agent.policy.load_state_dict(torch.load('./PPO.pth'))
...
for r in range(config.R):
	...
	with open(config.home + '/results/SplitRLC_res.pkl','wb') as f:
				pickle.dump(res,f)
