...
class Client(Communicator):
	def __init__(self, index, ip_address, server_addr, server_port, datalen, model_name, split_layer):
		...
		#self.uninet = utils.get_model_AlexNet('Unit', self.model_name, config.model_len-1, self.device, config.model_cfg)
		#self.uninet = utils.get_model_LeNet('Unit', self.model_name, config.model_len-1, self.device, config.model_cfg)
	...
	def initialize(self, split_layer, offload, first, LR):
		if offload or first:
			...
			#self.net = utils.get_model_AlexNet('Client', self.model_name, self.split_layer, self.device, config.model_cfg)
			#self.net = utils.get_model_LeNet('Client', self.model_name, self.split_layer, self.device, config.model_cfg)
			logger.debug(self.net)
			self.criterion = nn.CrossEntropyLoss()

		#self.optimizer = optim.Adagrad(self.net.parameters(), lr=LR)
		self.optimizer = optim.SGD(self.net.parameters(),lr=LR, momentum=0.8, weight_decay=0.0015)
	...
	def train(self, trainloader):
		...
		if self.split_layer == (config.model_len -1): # No offloading training
			for batch_idx, (inputs, targets) in enumerate(tqdm.tqdm(trainloader)):
				...
				loss = self.criterion(outputs, targets) + 0.001 * proximal_term
				loss.backward()
				self.optimizer.step()
